...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ohmcoin.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: ohmcoind.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeOhmcoinamount=<n> " + _("Keep N Ohmcoin anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Ohmcoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Ohmcoin is probably already running."), strDataDir));
    ...
    if (fDaemon)
        fprintf(stdout, "Ohmcoin server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Ohmcoin") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Ohmcoin to complete") << "\n";
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizeOhmcoinAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    /* Denominations
       ...
       For example:
       1Ohmcoin+1000 == (.1Ohmcoin+100)*10
       10Ohmcoin+10000 == (1Ohmcoin+1000)*10
    */
